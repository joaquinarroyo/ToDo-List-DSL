...
checkFilter (And e1 e2) = do f1 <- checkFilter e1
                             f2 <- checkFilter e2
                             return $ And f1 f2
checkFilter (Or e1 e2) = do f1 <- checkFilter e1
                            f2 <- checkFilter e2
                            return $ Or f1 f2
...
search' f [] = []
search' f ((_,t):ts) = if evalFilter t f
                        then t : search' f ts
                        else search' f ts
...
searchRecursive' f [] = []
searchRecursive' f ((_,t):ts) = if evalFilter t f
